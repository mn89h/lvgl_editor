...
lv_obj_t * screen1_create(void)
{
    ...
    lv_anim_timeline_t ** at_array = lv_malloc(sizeof(lv_anim_timeline_t *) * _SCREEN1_TIMELINE_CNT);
    at_array[SCREEN1_TIMELINE_SCREEN_OPEN] = timeline_screen_open_create(lv_obj_0);
    ...
    lv_obj_set_height(lv_chart_0, 200);
    lv_chart_series_t * lv_chart_series_0 = lv_chart_add_series(lv_chart_0, lv_color_hex(0xff0000), LV_CHART_AXIS_PRIMARY_Y);
    static const int32_t lv_chart_0_values[] = {10,, 20,, 30,, 40};
    lv_chart_set_series_values(lv_chart_0, lv_chart_series_0, lv_chart_0_values, 4);
    ...
    lv_obj_add_play_timeline_event(lv_obj_0, LV_EVENT_SCREEN_LOADED, screen1_get_timeline(lv_obj_0, SCREEN1_TIMELINE_SCREEN_OPEN), 0, false);
    ...
    lv_obj_set_name(lv_obj_0, "screen1");
...
{
    if (timeline_id >= _SCREEN1_TIMELINE_CNT) {
        LV_LOG_WARN("screen1 has no timeline with %d ID", timeline_id);
        return NULL;
    }
...
static void free_timeline_event_cb(lv_event_t * e)
{
    ...
    uint32_t i;
    for(i = 0; i < _SCREEN1_TIMELINE_CNT; i++) {

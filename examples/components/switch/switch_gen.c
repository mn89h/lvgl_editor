...
lv_obj_t * switch_create(lv_obj_t * parent)
{
    ...
    if (!style_inited) {
        ...
        lv_style_set_bg_color(&switch_main_light, BG_SECONDARY_LIGHT);

        lv_style_init(&switch_main_dark);
        lv_style_set_bg_color(&switch_main_dark, BG_SECONDARY_DARK);
    ...
    lv_obj_bind_style(lv_switch_0, &switch_knob_dark, LV_PART_KNOB, &theme, 1);
    lv_obj_bind_style(lv_switch_0, &switch_main_dark, LV_PART_MAIN, &theme, 1);
    lv_obj_bind_style(lv_switch_0, &switch_indicator_dark, LV_PART_INDICATOR | LV_STATE_CHECKED, &theme, 1);
